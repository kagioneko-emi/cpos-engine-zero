...
5. Correct remote: `origin https://github.com/kagioneko/cpos-engine-zero.git`; latest pushed code commit before this memo: `4ee3d2e Add agent adapter to demo readiness`.
...
Latest pushed code commit: `4ee3d2e Add agent adapter to demo readiness`
...
3. Then choose one:
   - **Demo capture path**: run `/demo/fixture`, open dashboard, capture Competitive Demo Readiness + External Agent Adapter Queue + Human Escalation + Ready-to-Run + Flow Graph.
   - **Release path**: update release/checklist docs to `318 passed`, then prepare final `v0.1.0` only ifねこさん explicitly says so.
