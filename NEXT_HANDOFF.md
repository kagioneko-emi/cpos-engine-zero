...
- Sandbox Patch Execution run added in `cpos/sandbox_patch_runner.py` with API route `POST /sandbox/executions/<task_id>/run`. It requires an approved sandbox patch execution plan, applies the patch only in an ephemeral workspace copy, validates command hashes before running, and stores hashes/exit codes/status only; raw command output stays out of Task Tape.
